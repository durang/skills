...
def subprocess_env(extra: dict | None = None) -> dict:
    env = {**os.environ}
    path_parts = [str(BUN_BIN_DIR), str(HOME / ".local/bin"), "/usr/local/bin", "/usr/bin", "/bin"]
    current_path = env.get("PATH")
    if current_path:
        path_parts.append(current_path)
    env["PATH"] = os.pathsep.join(dict.fromkeys(path_parts))
    env.setdefault("HOME", str(HOME))
    env.setdefault("GBRAIN_HOME", str(HOME))
    if extra:
        env.update(extra)
    return env
...
{
  "decisions":   [{"slug":"decisions/<kebab>","title":"...","summary":"<2-4 sentences>","links":["<related-slug>"],"effective_date":"YYYY-MM-DD"}],
  "insights":    [{"slug":"originals/<kebab>","title":"...","summary":"<user's exact phrasing>","links":["<related-slug>"],"effective_date":"YYYY-MM-DD"}],
  "entities":    [{"slug":"people/<kebab>|companies/<kebab>","title":"...","summary":"...","links":["<related-slug>"],"effective_date":null}],
  "concepts":    [{"slug":"concepts/<kebab>","title":"...","summary":"...","links":["<related-slug>"],"effective_date":null}],
  "skip_reason": null
}
...
- links: 1-3 slugs of OTHER signals in this same extraction, or well-known entities the item directly involves (e.g. "companies/openai"). Never self-link. Empty array if truly standalone.
- effective_date: the date the decision/insight/event actually happened (session date if today). null for timeless entities/concepts.
...
def call_via_claude_cli(transcript: str) -> dict:
    ...
    child_env = subprocess_env({"GBRAIN_HOOK_RUNNING": "1"})
    try:
        r = subprocess.run(
...
def call_via_api(api_key: str, transcript: str) -> dict:
    ...
    try:
        ...
        if detail:
            ...
            return {"skip_reason": f"api_http_{e.code}: {detail}"}
...
# Namespaces this hook is allowed to write. Must stay a subset of the active
# schema pack's page_types (~/.gbrain/schema-packs/<pack>/pack.json) — an
...
def write_to_gbrain(slug: str, title: str, body: str, session_id: str,
    ...
    if not SLUG_RE.match(slug):
        ...
        return False
    # A namespace is mandatory: `type` below is the first path segment, so a
    # bare slug ("google-maps-api-for-coords") becomes its own page type and
    # blows up type_proliferation (131 junk types by 2026-07-25). Reject
    # rather than guess the namespace — a lost signal beats a poisoned schema.
    if "/" not in slug or slug.split("/", 1)[0] not in ALLOWED_NAMESPACES:
        _log_write_failure(slug, "slug_missing_or_unknown_namespace")
        return False
    if not title or not body:
        _log_write_failure(slug, "empty_title_or_body")
        return False
    # Wikilinks feed gbrain's graph (extract reads [[slug]] from markdown).
    # Validate each link like a slug; drop self-links; cap at 3.
    clean_links = []
    for l in (links or [])[:3]:
        l = (l or "").strip()
        if l and l != slug and len(l) <= 120 and SLUG_RE.match(l):
            clean_links.append(l)
    date_line = ""
    if effective_date and DATE_RE.match(effective_date.strip()):
        date_line = f"effective_date: {effective_date.strip()}\n"
    related = ""
    if clean_links:
...
def main():
    ...
    for cat in ("decisions","insights","entities","concepts"):
        for item in (signals.get(cat) or []):
            ...
            if write_to_gbrain(slug, title, body, session_id, links, effective_date):
                counts[cat] += 1
            else:
                write_failures += 1
